...
function Get-FindInfoFromPattern {
    ...
    if ((Test-Rooted -Path $Pattern)) {
        # The pattern is rooted.
        $findPath = $joinedSegments
    }
    elseif ($joinedSegments) {
        # The pattern is not rooted, and literal segements were found.
        $findPath = [System.IO.Path]::Combine($DefaultRoot, $joinedSegments)
    }
    else {
        # The pattern is not rooted, and no literal segements were found.
        $findPath = $DefaultRoot
    }
...
function Get-FindResult {
    ...
    while ($stack.Count) {
        ...
        # Traverse.
        if (($item.Attributes -band 0x00000010) -eq 0x00000010) {
            # Directory
